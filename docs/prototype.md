...
![EnclosureOpen](/docs/images/prototype2/opened.jpeg)
![EnclosureClosed](/docs/images/prototype2/closed.jpeg)
![TestInstall](/docs/images/prototype2/testing.jpg)

(the wiring is wrong in the picture of the open enclosure, the yellow and green are swapped, I fixed that later)
...
See below for details about the old, larger enclosure, which is still available via OnShape. For the updated enclosure, the MOSFETS must be bent so they don't stick up past the Raspberry Pi Pico, but it shaves off a fairly good amount of height and is about the same width with the wings, it'd be noticeably smaller if you removed them from the model but I'm not sure if the outer four screws are enough or not. This update was also made in OnShape at the same link. This new enclosure doesn't use screws to hold down the PCB, instead the inner walls of the top of the enclosure hold it down and there are pins that the mounting holes on the PCB sit on, so that removes four screws and brass fittings. The design does unfortunately require support and was designed around a 0.4mm nozzle, if I make more updates to it I will try to reduce the support requirements, but in this case I had to decide between really long screws or using support.

The new enclosure is designed so that the PCB can be removed without removing the wires from the terminal blocks, and so that the pico's USB port is accessible even while the PCB sits in the bottom of the enclosure. The intent is to fill the wire openings with hot glue or epoxy or something similar to seal them, though I am also considering making a printable TPU gasket.

![OldVsNew](/docs/images/prototype2/oldVsNew.jpg)

The new one is black here because it was a test print in PLA, the final version was printed in a gray PETG.
...
| Name                                                      | Quantity | Link                                                  | Notes                                                                                                                                                                 |
| --------------------------------------------------------- | -------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M3 Heat-Set Inserts                                       | 6        | <https://www.amazon.com/dp/B0CS6VZYL8>                | 6 for the lid                                                                                                                                                         |
| M3x10mm bolts                                             | 6        | generic                                               | Longer will also work                                                                                                                                                 |
...
- Shorten the height of the enclosure (further) by bending the MOSFETs (or SMD components).
